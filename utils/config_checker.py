...
class ConfigChecker:
    ...
    def check_external_knowledge(self) -> Tuple[bool, Optional[str]]:
        """
        检查和加载外部知识库
        
        Returns:
            Tuple[bool, Optional[str]]: (是否成功, 错误信息或状态信息)
        """
        knowledge_path = settings.agent.external_knowledge_path
        
        if not knowledge_path or not knowledge_path.strip():
            return True, "未配置外部知识库 / No external knowledge configured"
        
        success, message = external_knowledge_manager.load_knowledge(knowledge_path)
    ...
    async def run_all_checks(self) -> bool:
        ...
        # 检查外部知识库
        print("  - 检查外部知识库... / Checking external knowledge...", end="", flush=True)
        knowledge_success, knowledge_message = self.check_external_knowledge()
        if knowledge_success:
            print(" ✅")
            if knowledge_message and "未配置" not in knowledge_message:
                print(f"    📚 {knowledge_message}")
        else:
            print(" ⚠️ ")
            print(f"    ⚠️  外部知识库警告 / External knowledge warning: {knowledge_message}")
            print(f"    ⚠️  系统将继续运行，但外部知识库功能将不可用")
            print(f"    ⚠️  System will continue, but external knowledge will be unavailable")
