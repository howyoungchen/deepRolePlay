...
def extract_current_user_input(messages: List[Dict[str, Any]]) -> Optional[str]:
    """
    从消息列表中自下而上查找用户消息里以 <drp_current_user_input> 包裹的内容。

    规则：
    - 仅检查 role == "user" 的消息
    - 自底向上查找，匹配到的第一对标签优先
    - 成对标签形式：<drp_current_user_input>... </drp_current_user_input>
    ...
    pattern = re.compile(r"<drp_current_user_input>([\s\S]*?)</drp_current_user_input>")
