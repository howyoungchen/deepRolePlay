...
class FastReActWorkflow:
    ...
    def _build_search_tools(self, messages: List[Dict[str, Any]]):
        ...
        # 检查并加载外部知识库
        if settings.agent.external_knowledge_path:
            try:
                with open(settings.agent.external_knowledge_path, 'r', encoding='utf-8') as f:
                    external_knowledge = f.read()
                # 将外部知识库内容添加到搜索文本前面
                search_text = f"=== 外部知识库 ===\n{external_knowledge}\n\n=== 对话历史 ===\n{search_text}"
                print(f"\\ 加载外部知识库: {settings.agent.external_knowledge_path}", flush=True)
            except Exception as e:
                print(f"\\ ⚠️ 无法加载外部知识库 {settings.agent.external_knowledge_path}: {e}", flush=True)
