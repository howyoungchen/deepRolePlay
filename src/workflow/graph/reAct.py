...
class ReActAgent:
    def __init__(self, model: AsyncOpenAI, max_iterations: int, system_prompt: str, user_input: str, tools_with_schemas: List[Dict[str, Any]], 
        ...
        # 收集本次运行的工具输出（按调用顺序）
        # 每个元素包含 {"name": 工具名, "content": 工具输出}
        self._collected_tool_messages: List[Dict[str, str]] = []

    def get_tool_outputs(self) -> List[Dict[str, str]]:
        """获取最近一次运行期间收集到的工具输出列表"""
        return list(self._collected_tool_messages)

    def get_tool_outputs_text(self) -> str:
        """将最近一次运行期间的工具输出合并为文本，包含工具名分隔"""
        if not self._collected_tool_messages:
            return ""
        parts = []
        for item in self._collected_tool_messages:
            name = item.get("name") or item.get("tool_name") or "unknown_tool"
    ...
    async def ainvoke(self) -> AsyncGenerator[str, None]:
        ...
        while iteration < self.max_iterations:
            ...
            for result in tool_results:
                ...
                # 收集工具输出
                self._collected_tool_messages.append({
