...
from utils.messages_process import extract_current_user_input
...
class ReActWorkflow:
    ...
    async def run(self, state: Dict[str, Any]) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            # 提取当前用户输入（必须存在）
            current_user_input = extract_current_user_input(messages)
            if not current_user_input or not current_user_input.strip():
                raise ValueError("未找到当前用户输入标记：请确保在最新的用户消息中包含 <drp_current_user_input>...</drp_current_user_input> 包裹。")
            ...
            # 将错误透传到前端（流式时会先输出一段错误文本），随后中断本次请求
            try:
                yield error_msg
            finally:
                raise
