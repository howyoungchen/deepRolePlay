...
#    - **搜索次数限制：最多执行6次re_search调用，请合理规划搜索策略**
#    - 搜索策略（按优先级）：
...
#    - **搜索次数限制：最多执行4次wikipedia_tool调用，选择最关键的实体进行外部搜索**
...
# **[实体名称]**
# [基于搜索结果为当前情景整合的记忆描述，特别强调本重要实体与其他重要实体的关联，最多200字]
...
# - re_search：
#   * 必须优先使用多实体联合搜索
#   * 每个重要实体都要与其他相关重要实体进行联合搜索
#   * 单实体搜索仅在联合搜索重要实体无结果时作为补充
# - wikipedia_tool：
#   * 用于搜索需要外部知识补充的实体或概念
...
# 重要限制：
# - 工具调用次数：re_search最多6次，wikipedia_tool最多4次
